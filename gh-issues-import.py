...
def init_config():
	...
	arg_parser.add_argument('-y', '--yes', action='store_true', help="Suppress confirmation questions.")
	...
	def get_credentials_for(which):
		if not config.has_option(which, 'username'):
			...
			elif ( (which == 'target') and (config.getboolean('settings', 'yes') or query.yes_no("Do you wish to use the same credentials for the target repository?") )):
...
def import_issues(issues):
	...
	if not config.getboolean('settings', 'yes') and not query.yes_no("Are you sure you wish to continue?"):
		sys.exit()
